...
from optim_Grassmann import LBFGS_Gr

def OneStepVarGr(Iter, T, Ham, dtype):

    TReal = torch.nn.Parameter(T.real, requires_grad=True)
    TImag = torch.nn.Parameter(T.imag, requires_grad=True)
    optimizer = LBFGS_Gr([TReal, TImag], lr=args.lr, max_iter=args.max_iter)
    ...
    def LossW(TReal, TImag, Ham):
        ...
        Id = torch.eye(args.D, dtype=dtype).contiguous().view(args.D * args.D)
        VarE = Id @ TMTMat @ RightVec / (Id @ RightVec)
